...
public:
	enum class Level {
		ERROR_ = LOGGER_LEVEL_ERROR,
		WARNING_ = LOGGER_LEVEL_WARNING,
		INFO_ = LOGGER_LEVEL_INFO,
		DEBUG_ = LOGGER_LEVEL_DEBUG,
		VERBOSE_ = LOGGER_LEVEL_VERBOSE
	};
	typedef std::function<void(char*, size_t)> PrintFun;
	typedef std::function<uint32_t()> TickFun;

	Logger(PrintFun printFun, TickFun tickFun, size_t bufferSize = 256) :
			printFun(printFun), tickFun(tickFun), bufferSize(bufferSize){
	}
	...
	void log(Level lvl, const char* TAG, const char* format, va_list args){
		...
		size_t writted = vsnprintf(buffer, bufferSize, format, args);
		write(lvl, TAG, buffer, writted);
	}
	...
	void error(const char* TAG, const char* format, va_list args){
	...
	void warning(const char* TAG, const char* format, va_list args){
	...
	void info(const char* TAG, const char* format, va_list args){
	...
	void debug(const char* TAG, const char* format, va_list args){
	...
	void verbose(const char* TAG, const char* format, va_list args){
